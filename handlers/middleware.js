...
exports.validateCompany = (req,res,next) => {
    const errors = feildValidation(req);
    if(errors){
      const message = errors[0].msg;
...
//middleware for company update form
exports.companyUpdateValid = (req,res,next) => {
  const errors = feildValidation(req);
  if(errors){
    const message = errors[0].msg;
    req.flash('error',message);
  ...
  //extra validation
  if(!req.body.state || !req.body.city){
    const message = 'State and City are mandatory.';
    req.flash('error',message);
    res.redirect('back');
    return;
  }
